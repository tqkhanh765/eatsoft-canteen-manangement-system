...
const getBarColor = (orders, maxOrders) => {
  ...
  if (ratio > 0.5) return '#FF6B00';
  if (ratio > 0.3) return '#FFC107';
  return '#2F9E44';
};

const PeakDayChart = ({ startDate, endDate }) => {
  ...
  useEffect(() => {
    const fetchPeakDay = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/orders/stats/peak-day?${params}`);
        if (!response.ok) throw new Error('Failed to fetch peak day data');
  ...
  const dataWithColors = data.map(item => ({ ...item, color: getBarColor(item.orders, maxOrders) }));
  ...
  if (error) return (
    <div className="manager-chart-container manager-chart-wide">
      <h3 className="manager-chart-title">PEAK DAY</h3>
      <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#EF4444' }}>Error: {error}</div>
    </div>
  );
    ...
    <div className="manager-chart-container manager-chart-wide">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={dataWithColors} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <XAxis dataKey="day" tick={{ fontSize: 11, fill: '#666' }} axisLine={false} />
          <YAxis hide />
          <Tooltip />
